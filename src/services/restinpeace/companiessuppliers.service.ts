...
export class CompaniesSuppliersService {
	...
	async getAll(companyID:string, listCount:number=-1, pageNumber:number=0, orderBy:string='name asc', timeoutSeconds:number=20000):Promise<EBaseEntity[]>{
		...
		const response = await this.authHttp.get(Defines.RestBaseURL + '/v1/SCompaniesSuppliersService/GetAll'+'/'+companyID, {params: params}).toPromise().catch(Helper.handleError);
		return response;
	}

	async getCount(companyID:string, timeoutSeconds:number=20000):Promise<number>{
		const response = await this.authHttp.get(Defines.RestBaseURL + '/v1/SCompaniesSuppliersService/GetCount'+'/'+companyID).toPromise().catch(Helper.handleError);
		return response;
	}

	async getByID(id:string, timeoutSeconds:number=20000):Promise<EBaseEntity>{
		const response = await this.authHttp.get(Defines.RestBaseURL + '/v1/SCompaniesSuppliersService/GetByID'+'/'+id).toPromise().catch(Helper.handleError);
		return response;
	}
	...
	async search(txt:string, listCount:number=-1, pageNumber:number=0, orderBy:string='name asc', timeoutSeconds:number=20000):Promise<EBaseEntity[]>{
		...
		if(pageNumber!=null) params= params.set('pageNumber', pageNumber.toString());
		if(orderBy!=null) params= params.set('orderBy', orderBy.toString());
		const response = await this.authHttp.get(Defines.RestBaseURL + '/v1/SCompaniesSuppliersService/Search', {params: params}).toPromise().catch(Helper.handleError);
		return response;
	}

	async save(eBaseEntity:EBaseEntity, timeoutSeconds:number=20000):Promise<string>{
		const response = await this.authHttp.put(Defines.RestBaseURL + '/v1/SCompaniesSuppliersService/Save', JSON.stringify(eBaseEntity), Defines.httpOptions).toPromise().catch(Helper.handleError);
		return response;
	}

	async remove(id:string, timeoutSeconds:number=20000):Promise<boolean>{
		const response = await this.authHttp.delete(Defines.RestBaseURL + '/v1/SCompaniesSuppliersService/Remove'+'/'+id).toPromise().catch(Helper.handleError);
		return response;
	}
